...
class UniversalService extends AutomaticListConvertable {
  ...
  @Path("/inuse/{barcode}")
  @Transactional
  def barcodeInUse(@PathParam("barcode") barcode: String): AbstractResultDTO = ValidateBarcode(barcode) match {
    ...
    case ValidBarcode(code@Barcode(Prefix.BDKJ | Prefix.Spielekreis, counter, checksum)) => gamesDao.selectActivatedByBarcode(code) match {
      case Some(game) => lendGameDao.selectLendGameByGame(game) match {
        case Some(lendGame) => LendingEntityInUseDTO(lendGame.toGameDTO, GameInUseDTO(lendGame.toIdentityCardDTO, lendGame.toEnvelopeDTO))
    ...
    case ValidBarcode(code@Barcode(Prefix.IdentityCards, counter, checksum)) => identityCardDao.selectActivatedByBarcode(code) match {
      case Some(idcard) => lendIdentityCardDao.selectCurrentByIdentityCard(idcard) match {
        case Some(lic: LendIdentityCard) => LendingEntityInUseDTO(lic.toIdentityCardDTO, IdentityCardInUseDTO(lic.toEnvelopeDTO, lic.toGameDTO))
    ...
    case ValidBarcode(code@Barcode(Prefix.Envelopes, counter, checksum)) => envelopeDao.selectActivatedByBarcode(code) match {
      case Some(envelope) => lendIdentityCardDao.selectCurrentByEnvelope(envelope) match {
        case Some(lic: LendIdentityCard) => LendingEntityInUseDTO(lic.toEnvelopeDTO, EnvelopeInUseDTO(lic.toIdentityCardDTO, lic.toGameDTO))
  ...
  def gamesInformation(gameInformationRequest: GameInformationRequestDTO): GameInformationDTO = gameInformationRequest match {
    case request@GameInformationRequestDTO(_, _, _, _, _, _, _) => {
      val foundGames = gamesDao.selectAllGamesWithRequirements(
        ...
        .map { game => game.toLendGameStatusDTO(lendGameDao.selectLendGameByGame(game)) }.toArray
  ...
  @Transactional
  def statusInformation(@PathParam("barcode") barcode: String): AbstractResultDTO = ValidateBarcode(barcode) match {
    // The given barcode is gamelike
    case ValidBarcode(code@Barcode(Prefix.BDKJ | Prefix.Spielekreis, counter, checksum)) => gamesDao.selectActivatedByBarcode(code) match {
      case None => LendingEntityNotExistsDTO(barcode)
      case Some(game) => lendGameDao.selectLendGameByGame(game) match {
        case Some(lendGame: LendGame) => InformationDTO(Array(lendGame.toGameDTO), lendGame.toIdentityCardDTO, lendGame.toEnvelopeDTO)
    ...
    case ValidBarcode(code@Barcode(Prefix.IdentityCards, counter, checksum)) => identityCardDao.selectActivatedByBarcode(code) match {
      case None => LendingEntityNotExistsDTO(barcode)
      case Some(idcard) => lendIdentityCardDao.selectCurrentByIdentityCard(idcard) match {
        case Some(lic: LendIdentityCard) => InformationDTO(lic.toGameDTO, lic.toIdentityCardDTO, lic.toEnvelopeDTO)
