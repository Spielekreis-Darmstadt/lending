...
@Path("/issue")
@RequestScoped
class IssueGamesService extends AutomaticListConvertable {
  ...
  @Path("/games")
  @Transactional
  def issueGames(issueGameRequest: IssueGameRequestDTO): AbstractResultDTO =
    issueGameRequest match {
      case IssueGameRequestDTO(Some(identityCardBarcode), gameBarcodes, Some(limited)) =>
        (ValidateBarcode(identityCardBarcode), gameBarcodes) match {
          case (ValidBarcode(identityCardBarcode), gameBarcodes @ Array(_, _*)) =>
            gameBarcodes.find {
              ...
              case None => {
                val identityCard: Option[Either[LendIdentityCard, IdentityCard]] = findIdentityCard(
                  identityCardBarcode
                )

                val games: List[Option[Game]] =
                  gameBarcodes.map { gameBarcode =>
                    gamesDao.selectActivatedByBarcode(new Barcode(gameBarcode))
                  }.toList
                ...
                (identityCard, notActivatedGameBarcodePairs, alreadyBorrowedGames) match {
                  ...
                  case (Some(Left(lic @ LendIdentityCard(_, _, _, _, _))), Nil, Nil)
                      if lic.hasNoCurrentLendGames => {
                  ...
                  case (Some(Left(lendIdentityCard)), _, alreadyBorrowedGame :: _) =>
                    LendingEntityInUseDTO(
                      alreadyBorrowedGame.toGameDTO,
                      GameInUseDTO(
                        alreadyBorrowedGame.toIdentityCardDTO,
                        alreadyBorrowedGame.toEnvelopeDTO
                      )
                    )

                  // the identity card has currently at least one borrowed game
                  case (Some(Left(lic @ LendIdentityCard(_, _, _, _, _))), _, _)
                      if limited && lic.hasCurrentLendGames =>
                    IdentityCardHasIssuedGamesDTO(lic.toIdentityCardDTO, lic.toGameDTO)

                  // the identity card is currently not issued
                  case (Some(Right(identityCard)), _, _) =>
                    LendingEntityInUseDTO(identityCard.toIdentityCardDTO, NotInUseDTO())
