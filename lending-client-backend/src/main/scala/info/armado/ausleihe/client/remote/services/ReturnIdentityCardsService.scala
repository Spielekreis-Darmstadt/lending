...
class ReturnIdentityCardsService {
  ...
  ): AbstractResultDTO = returnIdentityCardRequest match {
    case ReturnIdentityCardRequestDTO(Some(identityCardBarcode), Some(envelopeBarcode)) =>
        ...
        case (ValidBarcode(identityCardBarcode), ValidBarcode(envelopeBarcode)) =>
          (findIdentityCard(identityCardBarcode), findEnvelope(envelopeBarcode)) match {
            ...
            /**
              ...
              */
            case (
                Some(Left(lendIdentityCard @ LendIdentityCard(_, _, _, _, _))),
                Some(Left(lendEnvelope))
                ) if lendIdentityCard == lendEnvelope && lendIdentityCard.hasNoCurrentLendGames => {
              lendIdentityCardDao.returnIdentityCard(lendIdentityCard)
              ReturnIdentityCardSuccessDTO(
                lendIdentityCard.toIdentityCardDTO,
                lendIdentityCard.toEnvelopeDTO
              )
            }
            ...
            case (
                Some(Left(lendIdentityCard @ LendIdentityCard(_, _, _, _, _))),
                Some(Left(lendEnvelope))
                ) if lendIdentityCard == lendEnvelope && lendIdentityCard.hasCurrentLendGames =>
              IdentityCardHasIssuedGamesDTO(
                lendIdentityCard.toIdentityCardDTO,
                lendIdentityCard.toGameDTO
              )

            // the identity card is not issued to the given envelope
            case (Some(Left(lendIdentityCard)), Some(Left(otherLendIdentityCard)))
                if lendIdentityCard != otherLendIdentityCard =>
              IdentityCardEnvelopeNotBoundDTO(
                lendIdentityCard.toIdentityCardDTO,
                otherLendIdentityCard.toEnvelopeDTO,
                lendIdentityCard.toEnvelopeDTO
              )

            case (Some(Right(identityCard)), _) =>
              LendingEntityInUseDTO(identityCard.toIdentityCardDTO, NotInUseDTO())
            case (_, Some(Right(envelope))) =>
              LendingEntityInUseDTO(envelope.toEnvelopeDTO, NotInUseDTO())
