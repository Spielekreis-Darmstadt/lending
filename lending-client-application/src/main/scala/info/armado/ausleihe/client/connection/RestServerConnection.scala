...
object RestServerConnection extends ServerConnectionInterface {
  ...
  def searchGames(searchTerm: String, title: String, author: String, publisher: String, minimumAge: Integer, playerCount: Integer, gameDuration: Integer): Try[GameInformationDTO] = {
    Try(server.path("rest").path("info").path("gamesinformation").request(MediaType.APPLICATION_XML)
      .post(Entity.entity(GameInformationRequestDTO(searchTerm, title, author, publisher, playerCount, minimumAge, gameDuration), MediaType.APPLICATION_XML), classOf[GameInformationDTO]))
  }
  ...
  def lendGames(identityCardBarcode: String, gameBarcodes: Array[String], limited: Boolean): Try[AbstractResultDTO] = {
    Try(server.path("rest").path("issue").path("games").request(MediaType.APPLICATION_XML)
      .post(Entity.entity(IssueGameRequestDTO(identityCardBarcode, gameBarcodes, limited), MediaType.APPLICATION_XML), classOf[AbstractResultDTO]))
  }
  ...
  def lendIdentityCard(identityCardBarcode: String, envelopeBarcode: String): Try[AbstractResultDTO] = {
    Try(server.path("rest").path("issue").path("identitycard").request(MediaType.APPLICATION_XML)
      .post(Entity.entity(IssueIdentityCardRequestDTO(identityCardBarcode, envelopeBarcode), MediaType.APPLICATION_XML), classOf[AbstractResultDTO]))
  }
  ...
  def returnGame(gameBarcode: String): Try[AbstractResultDTO] = {
    Try(server.path("rest").path("return").path("games").request(MediaType.APPLICATION_XML)
      .post(Entity.entity(ReturnGameRequestDTO(gameBarcode), MediaType.APPLICATION_XML), classOf[AbstractResultDTO]))
  }
  ...
  def returnIdentityCard(identityCardBarcode: String, envelopeBarcode: String): Try[AbstractResultDTO] = {
    Try(server.path("rest").path("return").path("identitycard").request(MediaType.APPLICATION_XML)
      .post(Entity.entity(ReturnIdentityCardRequestDTO(identityCardBarcode, envelopeBarcode), MediaType.APPLICATION_XML), classOf[AbstractResultDTO]))
  }
}