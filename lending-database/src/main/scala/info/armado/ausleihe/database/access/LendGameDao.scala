...
class LendGameDao extends LendEntityDao[LendGame, Integer](classOf[LendGame]) {
  ...
  def selectCurrentLendIdentityCardByGame(game: Game): Option[LendIdentityCard] =
    Try(em.createQuery("select lg.lendIdentityCard from LendGame lg where lg.game = :game and lg.returnTime is null", classOf[LendIdentityCard])
      .setParameter("game", game).getSingleResult).toOption
  ...
  def hasLendIdentityCardCurrentlyLendGames(lic: LendIdentityCard): Boolean = {
    val count: Long = em.createQuery("select count(*) from LendGame lg where lg.lendIdentityCard = :lic and lg.returnTime is null", classOf[JLong])
  ...
  /**
    ...
    */
  @Transactional
  def issueGames(games: List[Game], lic: LendIdentityCard): Unit = insert(games.map(LendGame(_, lic, LocalDateTime.now())))
  ...
  /**
    ...
    */
  @Transactional
  def returnGame(game: Game): Unit = selectLendGameByGame(game).foreach(lendGame => returnGame(lendGame))
