...
class LendIdentityCardDao extends LendEntityDao[LendIdentityCard, Integer](classOf[LendIdentityCard]) {
  /**
    ...
    *
    * @param idCard The identity card which should be checked if it's issued
    * @return True if the given identtiy card is currently issued
    */
  def isIdentityCardIssued(idCard: IdentityCard): Boolean = {
    val count = em
      .createQuery("select count(*) from LendIdentityCard lic where lic.identityCard = :idCard and lic.returnTime is null", classOf[JLong])
  ...
  def isEnvelopeIssued(envelope: Envelope): Boolean = {
    val count = em
      .createQuery("select count(*) from LendIdentityCard lic where lic.envelope = :envelope and lic.returnTime is null", classOf[JLong])
  ...
  /**
    ...
    *
    * @param idCard   The identity card, which belongs to the searched LendIdentityCard object
    * @param envelope The envelope, which belongs to the searched LendIdentityCard object
  ...
  def selectCurrentByIdentityCardAndEnvelope(idCard: IdentityCard, envelope: Envelope): Option[LendIdentityCard] =
    Try(
      em.createQuery("from LendIdentityCard lic where lic.identityCard = :idCard and lic.envelope = :envelope and lic.returnTime is null", classOf[LendIdentityCard])
        .setParameter("idCard", idCard).setParameter("envelope", envelope).getSingleResult
    ).toOption
  ...
  /**
    ...
    *
    * @param idCard The identity card whose currently LendIdentityCard object is searched
    * @return The current LendIdentityCard object belonging to the given identity card, or null if no such object exists
    */
  def selectCurrentByIdentityCard(idCard: IdentityCard): Option[LendIdentityCard] =
    Try(
      em.createQuery("from LendIdentityCard lic where lic.identityCard = :idCard and lic.returnTime is null", classOf[LendIdentityCard])
        .setParameter("idCard", idCard).getSingleResult
    ).toOption
  ...
  /**
    ...
    * @return The current [[LendIdentityCard]] object belonging to the given identity card barcode
    */
  def selectCurrentByIdentityCardBarcode(barcode: Barcode): Option[LendIdentityCard] =
    Try(
      em.createQuery("from LendIdentityCard lic where lic.identityCard.barcode = :barcode and lic.returnTime is null", classOf[LendIdentityCard])
  ...
  /**
    ...
    * the given envelope-
    *
    * @param envelope The envelope, whose current LendIdentityCard object is requested
    * @return The current LendIdentityCard object belonging to the given envelope, or null if no such object exists
    */
  def selectCurrentByEnvelope(envelope: Envelope): Option[LendIdentityCard] =
    Try(
      em.createQuery("from LendIdentityCard lic where lic.envelope = :envelope and lic.returnTime is null", classOf[LendIdentityCard])
        .setParameter("envelope", envelope).getSingleResult
    ).toOption
  ...
  def selectCurrentByEnvelopeBarcode(barcode: Barcode): Option[LendIdentityCard] =
    Try(
      em.createQuery("from LendIdentityCard lic where lic.envelope.barcode = :barcode and lic.returnTime is null", classOf[LendIdentityCard])
        .setParameter("barcode", barcode).getSingleResult
    ).toOption
  ...
  /**
    ...
    *
    * @param idCard   The identity card to be issued to the given envelope
    * @param envelope The envelope to be bound to the given identity card
    */
  @Transactional
  def issueIdentityCard(idCard: IdentityCard, envelope: Envelope): Unit =
    insert(LendIdentityCard(idCard, envelope, LocalDateTime.now))
  ...
  /**
    ...
    *
    * @param idCard   The to be unbounded identity card
    * @param envelope The to be unbounded envelope
    */
  @Transactional
  def returnIdentityCard(idCard: IdentityCard, envelope: Envelope): Unit =
    selectCurrentByIdentityCardAndEnvelope(idCard, envelope).foreach(returnIdentityCard)

  /**
    * Sets a given [[LendIdentityCard]] object as returned
    *
    * @param lendIdentityCard The [[LendIdentityCard]] object to be marked as returned
    */
  @Transactional
  def returnIdentityCard(lendIdentityCard: LendIdentityCard): Unit = {
    lendIdentityCard.returnTime = LocalDateTime.now

    update(lendIdentityCard)
  }

  /**
    * Updates the owner of a given [[LendIdentityCard]] object to the a given owner
    *
    * @param lendIdentityCard The [[LendIdentityCard]] object, whose owner should be updated
